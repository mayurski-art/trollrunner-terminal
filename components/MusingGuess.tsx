...
export default function MusingGuess({
  musingId,
  session,
}: {
  musingId: string;
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        const res = await fetch(`/api/musing-guess?musingId=${encodeURIComponent(musingId)}`, {
          headers,
        });
  ...
  async function submitGuess(e: React.FormEvent) {
    ...
    try {
      ...
      const res = await fetch("/api/musing-guess", {
        method: "POST",
        headers,
        body: JSON.stringify({ musingId, guess }),
      });
      const data = await res.json();
      if (!res.ok) {
    ...
    <div className="mt-3 pt-3 border-t border-dim">
      {guessState?.resolved ? (
        <p className={`text-xs ${guessState.correct ? "text-problem" : "text-dim"}`}>
          {guessState.correct
            ? `[ you got it — +${cost} refunded and PROBLEMS to spare ]`
            : "[ not this time — the muse has gone quiet on this one ]"}
        </p>
      ) : (
        <form onSubmit={submitGuess} className="space-y-1.5">
          <p className="text-dim text-xs">
            what's it circling? ({cost} PROBLEM{cost === 1 ? "" : "S"} ·{" "}
            {maxAttempts - (guessState?.attempts ?? 0)} guess
