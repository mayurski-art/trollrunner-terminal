...
// Read-only: current guess state for this post/user, plus whether it's
// guessable at all (posts before clue_tag existed never will be).
export async function GET(request: Request) {
  ...
  const [{ data: post }, { data: guess }, wallet] = await Promise.all([
...
export async function POST(request: Request) {
  ...
  let body: { postId?: string; guess?: string };
  try {
  ...
  if (!guessRow) {
    ...
    if (insertError || !inserted) {
      ...
      await applyWalletDelta(supabase, userId, wallet, GUESS_COST, "post_guess_refund");
